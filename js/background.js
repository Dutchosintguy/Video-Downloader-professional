...
vd.incrementDownloadCount = function () {
  ...
  if(numberOfDownloads == 3){
    if ( confirm("You have downloaded multiple videos with Video Downloader professional. Please share your experience with others and make a review for us.")) {
      chrome.tabs.create({"url":"http://videodownloader.io/reviews/","selected":true}, function(tab){});
    }
  }
  if(numberOfDownloads == 7){
    if ( confirm("if you like what we do and can support our work, please make a donation so we can keep on making it even better.")) {
      chrome.tabs.create({"url":"http://videodownloader.io/contribute/","selected":true}, function(tab){});
    }
  }
};
...
vd.showYoutubeWarning = function () {
  chrome.tabs.create({"url": "http://bit.ly/youtubedown007","selected":true}, function(tab){})
};
